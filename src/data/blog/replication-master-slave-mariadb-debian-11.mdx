---
title: 'Replcation Master Slave MariaDB on Debian 11'
...
Replication Master-Slave adalah metode yang digunakan untuk menyalin data dari satu server basis data **Master** ke server basis data lainnya **Slave**. Metode ini digunakan untuk meningkatkan ketahanan sistem dan meningkatkan kinerja aplikasi dengan memperbolehkan beberapa server untuk membaca data dari satu sumber.
![Thumbnail](https://ik.imagekit.io/taufik/blog/replication-master-slave-mariadb-debian-11/thumbnail.png 'Thumbnail')
...
+------------------+----------+--------------+------------------+
```
Selesai kalia bisa keluar dari MariaDB, dan melanjutkan konfigurasi pada node slave
```mysql
EXIT;
```
...
```mysql
USE test_database;
CREATE TABLE table-database (id int, name varchar(20), surname varchar(20));
```
Isikan data pada tabel table-database.
```mysql
INSERT INTO table-database VALUES (1,"Taufik Crisnawan Santosa","Taufik");
INSERT INTO table-database VALIES (2,"Muhammad Ichsanul Fadhil","Ichsan");
```
...
+--------------------------+
```
Verifikasi kembali data yang ada pada table-database:
```mysql
+------+--------------------------+---------+
| id   |           name           | surname |
+------+--------------------------+---------+
